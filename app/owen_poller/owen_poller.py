...
class SensorsPoller:
    ...
    def _create_sensor(self, sensor_settings: dict[str, Any]) -> Sensor:
        ...
        return Sensor(
            name=sensor_name,
            device=device(
                addr=sensor_settings['addr'], addr_len=sensor_settings.get('addr_len', 8)
            ),
    ...
    async def sensors_update(self):
        ...
        for sensor in self.sensors.values():
            ...
            is_success = await asyncio.to_thread(sensor.update)
            if is_success:
    ...
    def get_list_readings(self, work_centers: list[str]) -> list[dict[str, Any]]:
        """
        Запрос данных по списку рабочих центров.
        Адаптировано для работы со словарями от ПВТ-110 и счетчиками от СИ8.
        ...
        for work_center in work_centers:
            ...
            if isinstance(sensor.device, ModbusPVT110):
    ...
    def _sort_sensors(self):
        """Сортирует сенсоры, чтобы первыми опрашивались СИ8, а ПВТ-110 в конце."""
        ci8_sensors = {k: v for k, v in self.sensors.items() if
                       isinstance(v.device, OwenCI8)}
        pvt_sensors = {k: v for k, v in self.sensors.items() if
                       isinstance(v.device, ModbusPVT110)}
        self.sensors = {**pvt_sensors, **ci8_sensors}
